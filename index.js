...
/**
   ...
   */function ht(t){return ct({...t,state:!0,attribute:!1})}var dt=Object.defineProperty,pt=(t,e,s,i)=>{for(var r,n=void 0,a=t.length-1;a>=0;a--)(r=t[a])&&(n=r(e,s,n)||n);return n&&dt(e,s,n),n};const ut=class extends nt{constructor(){super(...arguments),this.contactsWarning=5,this.contactsCritical=10,this.waitWarning=120,this.waitCritical=300,this.dataRefreshInterval=3e4,this.uiRefreshInterval=1e3,this.demoMode=!1,this.queueStats=[],this.contactDetails=new Map,this.isPanelOpen=!1,this.panelPosition={top:0,left:0},this.isLoading=!0,this.hasError=!1,this.errorMessage="",this.handleOutsideClick=t=>{this.isPanelOpen&&!t.composedPath().includes(this)&&(this.isPanelOpen=!1)}}connectedCallback(){super.connectedCallback(),this.initialize(),document.addEventListener("click",this.handleOutsideClick)}disconnectedCallback(){super.disconnectedCallback(),this.cleanup(),document.removeEventListener("click",this.handleOutsideClick)}async initialize(){try{this.isLoading=!0,this.hasError=!1,this.demoMode||(await this.fetchQueueStatistics(),this._dataRefreshTimer=setInterval(()=>this.fetchQueueStatistics(),this.dataRefreshInterval)),this._uiRefreshTimer=setInterval(()=>this.updateTemplate(),this.uiRefreshInterval)}catch(t){this.handleError(t)}}cleanup(){this._dataRefreshTimer&&clearInterval(this._dataRefreshTimer),this._uiRefreshTimer&&clearInterval(this._uiRefreshTimer)}async fetchQueueStatistics(){if(!this.token||!this.orgId)return this.hasError=!0,this.isLoading=!1,void(this.errorMessage="Missing token/orgId - check the widget attributes in the Desktop Layout configuration.");const t={Authorization:`Bearer ${this.token}`,"Content-Type":"application/json",Accept:"application/json"},e=Date.now(),s={query:`{\n        task(\n          from: ${e-864e5}\n          to: ${e}\n          filter: {\n            and: [\n              { isActive: { equals: true } }\n              { status: { equals: "parked" } }\n            ]\n          }\n          aggregations: [\n            { field: "id", type: count, name: "contacts" }\n            { field: "createdTime", type: min, name: "oldestStart" }\n          ]\n        ) {\n          tasks {\n            lastQueue { id name }\n            aggregation { name value }\n          }\n        }\n      }`};try{const e=await fetch("https://api.wxcc-us1.cisco.com/search",{method:"POST",headers:t,body:JSON.stringify(s),redirect:"follow"}),i=await e.json().catch(()=>null);if(!e.ok||i?.errors||i?.error)throw new Error(`Task search request failed (${e.status}): ${JSON.stringify(i?.errors||i?.error||i)}`);this.queueData=i?.data?.task?.tasks??[],this.updateTemplate(),this.isLoading=!1,this.hasError=!1,this.fetchContactDetails()}catch(i){this.handleError(i)}}async fetchContactDetails(){if(!this.token||!this.orgId)return;const t={Authorization:`Bearer ${this.token}`,"Content-Type":"application/json",Accept:"application/json"},e=Date.now(),s={query:`{\n        task(\n          from: ${e-864e5}\n          to: ${e}\n          filter: {\n            and: [\n              { isActive: { equals: true } }\n              { status: { equals: "parked" } }\n            ]\n          }\n        ) {\n          tasks {\n            id\n            createdTime\n            channelType\n            origin\n            lastQueue { id name }\n          }\n        }\n      }`};try{const e=await fetch("https://api.wxcc-us1.cisco.com/search",{method:"POST",headers:t,body:JSON.stringify(s),redirect:"follow"}),i=await e.json().catch(()=>null);if(!e.ok||i?.errors||i?.error)return void console.error("Contact details request failed:",e.status,i?.errors||i?.error);const r=i?.data?.task?.tasks??[],n=new Map;for(const t of r){const e=t.lastQueue?.id;if(!e)continue;const s=n.get(e)??[];s.push({id:t.id,createdTime:t.createdTime,channelType:t.channelType||"telephony",origin:t.origin||""}),n.set(e,s)}for(const t of n.values())t.sort((t,e)=>t.createdTime-e.createdTime);this.contactDetails=n}catch(i){console.error("Error fetching contact details:",i)}}updateTemplate(){this.queueData&&Array.isArray(this.queueData)?(this.queueStats=this.queueData.map(t=>{const e=t.aggregation?.find(t=>"contacts"===t.name)?.value||0,s=t.aggregation?.find(t=>"oldestStart"===t.name)?.value||0,i=s?Math.floor((Date.now()-s)/1e3):0;return{id:t.lastQueue?.id||"",name:t.lastQueue?.name||"Unknown Queue",contacts:e,waitTimeSeconds:i,status:this.calculateStatus(e,i)}}),this.queueStats.sort((t,e)=>{const s={critical:3,warning:2,ok:1};return(s[e.status]||0)-(s[t.status]||0)})):this.queueStats=[]}calculateStatus(t,e){return t>=this.contactsCritical||e>=this.waitCritical?"critical":t>=this.contactsWarning||e>=this.waitWarning?"warning":"ok"}formatWaitTime(t){const e=t%60;return`${Math.floor(t/60).toString().padStart(2,"0")}:${e.toString().padStart(2,"0")}`}getStatusText(t){switch(t){case"critical":return"Critical";case"warning":return"Warning";default:return"OK"}}getChannelIcon(t){switch((t||"").toLowerCase()){case"telephony":default:return"📞";case"chat":return"💬";case"email":return"📧";case"social":return"📱"}}togglePanel(t){if(this.isPanelOpen)return void(this.isPanelOpen=!1);const e=t.currentTarget.getBoundingClientRect();this.panelPosition={top:e.bottom+6,left:Math.max(8,Math.min(e.left,window.innerWidth-380))},this.isPanelOpen=!0}closePanel(){this.isPanelOpen=!1}handleError(t){console.error("Queue Statistics Error:",t),this.hasError=!0,this.isLoading=!1,this.errorMessage=t.message||"Failed to load queue statistics"}render(){return L`
    ...
    `:L`
      ...
      `}renderQueueCard(t,e){const s=0===t.contacts,i=!s;return L`
        ...
        ${i?L`
          <div class="card-body">
            ...
            <div class="contacts-section">
              ${this.renderContacts(this.contactDetails.get(t.id))}
            </div>
          </div>
        `:""}
      </div>
    `}renderContacts(t){return void 0===t?L`<div class="contacts-empty">Loading contact details…</div>`:0===t.length?L`<div class="contacts-empty">No individual contact details available</div>`:L`
      <div class="contacts-list">
        ${t.map(t=>L`
          <div class="contact-row">
            <span class="contact-channel">${this.getChannelIcon(t.channelType)}</span>
            <span class="contact-origin">${t.origin||"Unknown Caller"}</span>
            <span class="contact-wait">${this.formatWaitTime(Math.floor((Date.now()-t.createdTime)/1e3))}</span>
          </div>
        `)}
      </div>
